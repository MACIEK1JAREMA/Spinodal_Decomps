...
# Same but include errors on z from each repeat
...
for j in range(len(Ns)):
    ...
    for i in range(reps):
        average, kvals = MC.Sk_MCrun(Ns[j], J, T, dk, t0, tm, nth=nth)
        avgSk_norm = average / average[:, 0][:, None]
        ...
        exponents[j, i], _, _, _, _ = linreg(np.log(t_vals), np.log(L[1:]))
...
# Simple run with errors only from line, not from repeats
